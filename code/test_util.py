...
def recall(predict, target): #Sensitivity, Recall, true positive rate都一样
    ...
    return recall
def test_all_case(vnet, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    total_sen=0.0
    for image_path in tqdm(image_list):
        ...
        prediction, score_map = test_single_case(vnet, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        sen = recall(prediction, label[:])
    ...
    sen=total_sen/len(image_list)
    print('average metric is {}'.format(avg_metric))

    return avg_metric,sen


def test_single_case(vnet, image=None, stride_xy=None, stride_z=None, patch_size=None, num_classes=1):
    w, h, d = image.shape
